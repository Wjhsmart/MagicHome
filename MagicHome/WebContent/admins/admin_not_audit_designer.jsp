...
<body>
	<c:choose>
		...
		<c:when test="${empty(requestScope.designers) }">
			<h3 class="h3_red">暂无未审核的设计师</h3>
		</c:when>
		<c:otherwise>
			<div>
				<h3>根据条件查找：</h3>
				<form action="<%=path %>/admin/condition_query_not_audit_designer" method="post">
					<em>邮箱：</em>
				...
				<table class="table">
					...
					<c:forEach items="${requestScope.designers }" var="designer" varStatus="s">
						<tr>
							...
							<td>
								<a href="<%=path %>/admin/pass_audit_designer?email=${designer.e_mail }">通过审核</a>
								&nbsp;&nbsp;
								<a href="<%=path %>/admin/delete_not_audit_designer?email=${designer.e_mail }">删除</a>
			...
			<div style="text-align: right; padding: 60px;">
				<a href="<%=path %>/admin/query_not_audit_designer_page?pageSize=${requestScope.currentPageSize }">首页</a>
				<a href="<%=path %>/admin/query_not_audit_designer_page?pageNo=${requestScope.currentPage - 1 }&pageSize=${requestScope.currentPageSize }">上一页</a>
				<a href="<%=path %>/admin/query_not_audit_designer_page?pageNo=${requestScope.currentPage + 1 }&pageSize=${requestScope.currentPageSize }">下一页</a>
				<a href="<%=path %>/admin/query_not_audit_designer_page?pageNo=${requestScope.totalPage }&pageSize=${requestScope.currentPageSize }">尾页</a>
				...
				<span>页</span>
				<form id="skip" action="<%=path %>/admin/query_not_audit_designer_page" method="post"></form>
			</div>
		</c:otherwise>
	</c:choose>
</body>
