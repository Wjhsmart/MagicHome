...
<body>
	<c:choose>
		...
		<c:when test="${empty(requestScope.companys) }">
			<h3 class="h3_red">暂无已审核的装修公司</h3>
		</c:when>
		<c:otherwise>
			<div>
				<h3>根据条件查找：</h3>
				<form action="<%=path %>/admin/condition_query_company" method="post">
					<em>邮箱：</em>
					...
					<c:forEach items="${requestScope.companys }" var="company" varStatus="s">
						<tr>
							...
							<td>
								<c:choose>
									<c:when test="${company.status == 'Y'}">
										<a href="<%=path %>/admin/forbid_company?email=${company.e_mail }">禁用公司</a>
									</c:when>
									<c:when test="${company.status == 'N'}">
										<a href="<%=path %>/admin/start_company?email=${company.e_mail }">启用公司</a>
									</c:when>
									<c:otherwise>
			...
			<div style="text-align: right; padding: 60px;">
				<a href="<%=path %>/admin/query_company_page?pageSize=${requestScope.currentPageSize }">首页</a>
				<a href="<%=path %>/admin/query_company_page?pageNo=${requestScope.currentPage - 1 }&pageSize=${requestScope.currentPageSize }">上一页</a>
				<a href="<%=path %>/admin/query_company_page?pageNo=${requestScope.currentPage + 1 }&pageSize=${requestScope.currentPageSize }">下一页</a>
				<a href="<%=path %>/admin/query_company_page?pageNo=${requestScope.totalPage }&pageSize=${requestScope.currentPageSize }">尾页</a>
				...
				<span>页</span>
				<form id="skip" action="<%=path %>/admin/query_company_page" method="post"></form>
			</div>
		</c:otherwise>
	</c:choose>
</body>
