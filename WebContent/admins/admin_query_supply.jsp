...
<body>
	<c:choose>
		...
		<c:when test="${empty(requestScope.supplys) }">
			<h3 class="h3_red">暂无已审核的建材商</h3>
		</c:when>
		<c:otherwise>
			<div>
				<h3>根据条件查找：</h3>
				<form action="<%=path %>/admin/condition_query_supply" method="post">
					<em>邮箱：</em>
					...
					<c:forEach items="${requestScope.supplys }" var="supply" varStatus="s">
						<tr>
							...
							<td>
								<c:choose>
									<c:when test="${supply.status == 'Y'}">
										<a href="<%=path %>/admin/forbid_supply?email=${supply.e_mail }">冻结</a>
									</c:when>
									<c:when test="${supply.status == 'N'}">
										<a href="<%=path %>/admin/start_supply?email=${supply.e_mail }">解冻</a>
									</c:when>
									<c:otherwise>
			...
			<div style="text-align: right; padding: 60px;">
				<a href="<%=path %>/admin/query_supply_page?pageSize=${requestScope.currentPageSize }">首页</a>
				<a href="<%=path %>/admin/query_supply_page?pageNo=${requestScope.currentPage - 1 }&pageSize=${requestScope.currentPageSize }">上一页</a>
				<a href="<%=path %>/admin/query_supply_page?pageNo=${requestScope.currentPage + 1 }&pageSize=${requestScope.currentPageSize }">下一页</a>
				<a href="<%=path %>/admin/query_supply_page?pageNo=${requestScope.totalPage }&pageSize=${requestScope.currentPageSize }">尾页</a>
				...
				<span>页</span>
				<form id="skip" action="<%=path %>/admin/query_supply_page" method="post"></form>
			</div>
		</c:otherwise>
	</c:choose>
</body>
