	...
	<div class="about">
		<div class="container">
			<div class="about-top heading">
				<h1 style="margin-top: 10px;"><span style="color: green;">${requestScope.company.name }</span>的设计案例</h1>
				<h4>下面是<span style="color: green;">${requestScope.company.name }</span>的效果图</h4>
				<c:choose>
					<c:when test="${empty(requestScope.companyCases) }">
						<h4 style="color: red;">该装修公司暂时没有上传案例</h4>
					</c:when>
					<c:otherwise>
						<div class="about-bottom company_case">
							<c:forEach items="${requestScope.companyCases }" var="companyCase">
								<div class="col-md-4 about-left">
									...
									<h5><a href="<%=path %>/page/company_case_particular?id=${companyCase.id }">${companyCase.name }</a></h5>
									<p>${companyCase.des }</p>
								</div>
							</c:forEach>
		...
		<div style="text-align: right; padding: 60px;">
			<a href="<%=path %>/page/company_particular_case?pageSize=${requestScope.currentPageSize }&id=${requestScope.company.id }" style="color: #000;">首页</a>
			<a href="<%=path %>/page/company_particular_case?pageSize=${requestScope.currentPageSize }&pageNo=${requestScope.currentPage - 1 }&id=${requestScope.company.id }" style="color: #000;">上一页</a>
			<a href="<%=path %>/page/company_particular_case?pageSize=${requestScope.currentPageSize }&pageNo=${requestScope.currentPage + 1 }&id=${requestScope.company.id }" style="color: #000;">下一页</a>
			<a href="<%=path %>/page/company_particular_case?pageSize=${requestScope.currentPageSize }&pageNo=${requestScope.totalPage }&id=${requestScope.company.id }" style="color: #000;">尾页</a>
			...
			<span>页</span>
			<form id="skip" action="<%=path %>/page/company_particular_case?id=${requestScope.company.id }" method="post"></form>
		</div>
		<!-- 分页结束 -->
	</div>
	<!-- 中间部分结束 -->
