	...
	<div class="about">
		<div class="container">
			<div class="about-top heading">
				<h1 style="margin-top: 10px;">设计师案例</h1>
				<h4>下面是所有设计师的效果图</h4>
				<p>
					这里集结了全赣州最好的设计师，绝对满足您的各种需求，如果您觉得满意，就去联系他们吧
				</p>
				<div class="about-bottom company_case">
					<c:forEach items="${requestScope.designerCases }" var="designerCase">
						<div class="col-md-4 about-left">
							...
							<br />
							<c:choose>
								<c:when test="${designerCase.collected }">
									<span class="col" style="margin: 0px 120px;">已收藏</span>
								</c:when>
								<c:otherwise>
									<a href="<%=path %>/page/designer_case_col?id=${designerCase.id }" style="margin: 0px 120px;" class="col">收藏案例</a>
								</c:otherwise>
							</c:choose>
						</div>
					</c:forEach>
		...
		<div style="text-align: right; padding: 60px;">
			<a href="<%=path %>/page/designer_case?pageSize=${requestScope.currentPageSize }" style="color: #000;">首页</a>
			<a href="<%=path %>/page/designer_case?pageNo=${requestScope.currentPage - 1 }&pageSize=${requestScope.currentPageSize }" style="color: #000;">上一页</a>
			<a href="<%=path %>/page/designer_case?pageNo=${requestScope.currentPage + 1 }&pageSize=${requestScope.currentPageSize }" style="color: #000;">下一页</a>
			<a href="<%=path %>/page/designer_case?pageNo=${requestScope.totalPage }&pageSize=${requestScope.currentPageSize }" style="color: #000;">尾页</a>
			...
			<span>页</span>
			<form id="skip" action="<%=path %>/page/designer_case" method="post"></form>
		</div>
		<!-- 分页结束 -->
	</div>
	<!-- 中间部分结束 -->
